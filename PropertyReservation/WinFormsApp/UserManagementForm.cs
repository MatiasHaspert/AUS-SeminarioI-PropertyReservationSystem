...
namespace WinFormsApp
{
    public partial class UserManagementForm : Form
    {
        ...
        private void btnDetail_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                var detailTask = Program.UsersClient.GetUserByIdAsync(user.Id);
                detailTask.Wait();
        ...
        private async void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (IsSelf(user))
            {
                MessageBox.Show("No puede eliminarse a sí mismo.", "Operación no permitida",
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            if (MessageBox.Show($"¿Eliminar definitivamente al usuario '{user.Email}'?\r\nEsta acción no se puede deshacer.",
                    "Confirmar eliminación", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) != DialogResult.Yes) return;

            try
            {
                await Program.UsersClient.DeleteUserAsync(user.Id);
                await LoadUsersAsync();
            }
            catch (Exception ex)
            {
    ...
    // Diálogo embebido para CU-03 — Cambiar rol.
