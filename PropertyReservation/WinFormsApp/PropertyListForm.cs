...
namespace WinFormsApp
{
    public partial class PropertyListForm : Form
    {
        ...
        private async void btnEdit_Click(object sender, EventArgs e)
        {
            ...
            using var form = new PropertyForm(p.Id);
            if (form.ShowDialog(this) == DialogResult.OK)
                await LoadPropertiesAsync();
        }
        ...
        private async void btnHardDelete_Click(object sender, EventArgs e)
        {
            var p = GetSelected();
            if (p == null) return;

            if (MessageBox.Show(
                    $"¿ELIMINAR DEFINITIVAMENTE la propiedad '{p.Title}'?\r\n" +
                    "Esta acción es irreversible y solo procede si la propiedad no tiene reservas activas " +
                    "(Confirmed, PaymentUploaded, PendingPayment).",
                    "Eliminación definitiva",
                    MessageBoxButtons.YesNo, MessageBoxIcon.Warning) != DialogResult.Yes) return;

            try
            {
                await Program.PropertyClient.DeletePropertyAsync(p.Id, hard: true);
                await LoadPropertiesAsync();
            }
            catch (Exception ex)
            {
